...
from . import devices as devicess
...
def get_recommend_encoder_tile_size():
    if torch.cuda.is_available():
        total_memory = torch.cuda.get_device_properties(
            devicess.device).total_memory // 2**20
...
def perfcount(fn):
    def wrapper(*args, **kwargs):
        ...
        if torch.cuda.is_available():
            torch.cuda.reset_peak_memory_stats(devicess.device)
        devicess.torch_gc()
        gc.collect()
        ...
        gc.collect()
        if torch.cuda.is_available():
            vram = torch.cuda.max_memory_allocated(devicess.device) / 2**20
            torch.cuda.reset_peak_memory_stats(devicess.device)
            print(
...
class GroupNormParam:
    ...
    def summary(self):
        ...
        pixels = torch.tensor(
            self.pixel_list, dtype=torch.float32, device=devicess.device) / max_value
...
class VAEHook:
    ...
    def __call__(self, x):
        ...
        try:
            if self.to_gpu:
                self.net.to(devicess.get_optimal_device())
    ...
    def estimate_group_norm(self, z, task_queue, color_fix):
        ...
        for i in range(last_id + 1):
            ...
            if task[0] == 'pre_norm':
                ...
                tile = task[1](tile)
            try:
                devicess.test_for_nans(tile, "vae")
            except:
