    ...
    def validate(self, step: int) -> None:
        ...
        for i in range(num_validation_samples):
            ...
            validation_artifacts = self.validation_step(
                {"prompt": prompt, "image": image, "video": video_pil, "video_tensor": video, "ref_video": ref_video, "video_name": name_file}, pipe
            )
            ...
            # Process All Artifacts for Metrics
            current_metrics = {}
            processed_artifacts_for_saving = []

            for artifact_key, artifact_value in validation_artifacts:
                # Common processing
                if isinstance(artifact_value, torch.Tensor):
                     # [F, C, H, W] -> permute -> [B, C, F, H, W] equivalent
                     # Here logic assumes [C, F, H, W] or similar coming from validation_step?
                     # validation_step returns [Image] usually.
                     # Let's align with existing logic:
                     pass
                
                # We need to process each artifact individually
                if isinstance(artifact_value, torch.Tensor):
                    val_tensor = artifact_value.to(torch.float32)
                    val_tensor = val_tensor.permute(1, 0, 2, 3).contiguous() # [F, C, H, W] -> [C, F, H, W]? No existing was: [F, C, H, W] -> permute(1,0,2,3) -> [C, F, H, W]
                    target_f, _, target_h, target_w = ref_video.shape
                    val_tensor = val_tensor[:target_f, :, :target_h, :target_w]

                    _video_tensor = (val_tensor * 255).clamp(0, 255).to(torch.uint8)
                    pil_frames = [Image.fromarray(frame.permute(1, 2, 0).cpu().numpy()) for frame in _video_tensor]
                    
                    # For metrics
                    metric_tensor = val_tensor # [C, F, H, W] already ?
                    # Existing code: validation_artifacts_tensor
                else:
                    # PIL List
                    cropped_video = crop_padded_video(artifact_value, ref_video)
                    transform = transforms.ToTensor()
                    # [F, C, H, W]
                    val_tensor = torch.stack([transform(frame) for frame in cropped_video], dim=0).to(torch.float32)
                    pil_frames = cropped_video
                
                processed_artifacts_for_saving.append((artifact_key, pil_frames))

                if self.eval_metric_list is not None and ref_video is not None:
                     # ref_video is already [C, F, H, W] (from existing logic context?)
                     # Wait, ref_video preparation:
                     # ref_video = ref_video.to(torch.float32).clamp(0, 255) / 255.0
                     # We need to ensure ref_video is prepared ONCE.
                     pass 

                if self.eval_metric_list is not None and ref_video is not None:
                    # Ensure ref_video is float 0-1
                    # Logic below handles it, but let's do it per artifact comparison
                    try:
                        results_dict = evaluate_video_metrics(
                            pred_video=val_tensor,
                            ref_video=ref_video.to(torch.float32).clamp(0, 255) / 255.0 if ref_video.max() > 1.0 else ref_video,
                            models=self.eval_metric_list,
                            device=self.accelerator.device,
                            name=name_file,
                        )
                        # Prefix keys
                        prefix = f"{artifact_key}_" if artifact_key != "video" else ""
                        for k, v in results_dict.items():
                            current_metrics[f"{prefix}{k}"] = v
                    except Exception as e:
                        logger.warning(f"Failed to calculate metrics for artifact {artifact_key}: {e}")
            
            if current_metrics:
                all_processes_metrics.append(current_metrics)
            
            # Update validation_artifacts for saving loop
            validation_artifacts = processed_artifacts_for_saving
            ...
            for key, value in list(artifacts.items()):
                ...
                if (artifact_type != "image" and not artifact_type.startswith("video")) or artifact_value is None:
                    continue
                ...
                if key.startswith("artifact_"):
                     # Use artifact_type (e.g. "video", "video_api") as subfolder suffix
                     # result: validation_video, validation_video_api
                     subfolder = f"validation_{artifact_type}"
                else:
                     subfolder = "origin"
        ...
        all_artifacts_raw = gather_object(all_processes_artifacts)
        all_metrics_raw = gather_object(all_processes_metrics)
        
        # Flatten if needed (gather_object returns list of lists)
        all_artifacts = []
        for item in all_artifacts_raw:
            if isinstance(item, list): all_artifacts.extend(item)
            else: all_artifacts.append(item)
            
        all_metrics = []
        for item in all_metrics_raw:
            if isinstance(item, dict): all_metrics.append(item)
            elif isinstance(item, list): all_metrics.extend(item)

        if accelerator.is_main_process:
            if self.eval_metric_list is not None:
                # Gather all unique metric names from all processes
                metric_names = set()
                for m in all_metrics:
                    metric_names.update(m.keys())
                
                averaged_metrics = {
                    name: round(np.mean([m[name] for m in all_metrics if name in m]), 4) for name in metric_names
                }
                metrics_logs = {}
                for k, v in averaged_metrics.items():
                    metrics_logs[k] = v
                logger.info(f"Validation metrics at Step {step}: {json.dumps(metrics_logs, indent=4)}")
